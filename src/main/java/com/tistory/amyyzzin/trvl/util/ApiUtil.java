...
public class ApiUtil {
	...
	public StandardCodeResponseDto callStandardCodeApi() throws IOException {
		...
		urlBuilder.append("?" + URLEncoder.encode("serviceKey","UTF-8") + "=" + standardCodeKey); /*Service Key*/
		...
		if (response.isSuccessful()) {
			...
			//Gson 라이브러리를 통해 Json 형태로 변환한 API 결과를 PagingWifiInfo 에 저장
			Gson gson = new Gson();
			standardCodeResponseDto = gson.fromJson(jsonObject, StandardCodeResponseDto.class);
		}
